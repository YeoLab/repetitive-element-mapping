...
## pull bowtie2 scores from file
    ...
    if ($mapped_enst =~ /^(\S+)\_spliced/ || $mapped_enst =~ /^(\S+)\_withgenomeflank/) {
	$mapped_enst = $1;
    }
...
#    print "al $all_mapped_ensts\n";
    $read_hash{$r1name}{ensttype} = $ensttype;
}
close(BAM);
...
print "#READINFO\tRepMappingOnly\t$a_only\tUniqueMappingOnly\t$b_only\tIntersection\t$intersect\tUnion\t$union\n";
...
for my $s (@sorted) {
    ...
    print "$type\t$count_enst{$s}\t".sprintf("%.5f",$count_enst{$s} * 1000000 / $total_unique_mapped_read_num)."\t$s\t".join("|",@genes)."\n"; 
...
#       my ($allensg,$gid,$allenst) = split(/\t/,$line);
	...
	$type_label =~ s/\?$//;
        for my $enst (@ensts) {
            $enst2gene{$enst} = $gid;
	    $enst2gene{$enst."_antisense"} = $gid."_antisense";
...
#            $convert_enst2type{$enst."_antisense"} = $type_label."_antisense:".$priority_n;
            ...
            $convert_enst2type{$enst."_antisense"} = $type_label."_antisense";
            $priority_n++;
        }
    }
    close(F);
...
#        $peak_read_counts{$gene."_antisense"}{allpeaks} = 1;
        ...
        for my $i ($x..$y) {
            ...
            push @{$peaks{$chr}{$revstrand{$strand}}{$i}},$chr.":".$start."-".$stop.":".$strand.":".$gene."_antisense";
        }
    }
    close(F);
