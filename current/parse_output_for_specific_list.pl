...
my %desired_ensts;
my $output_label = "Y5";
my $desired_type_fi = "/home/elvannostrand/data/clip/CLIPseq_analysis/RNA_type_analysis/genelists.Y5RNA";
open(DES,$desired_type_fi);
for my $line (<DES>) {
    chomp($line);
    my ($des_ensg,$des_gn,$des_enst) = split(/\s+/,$line);
    $desired_genes{$des_gn} = 1;
    $desired_ensts{$des_enst} = "sense_".$output_label;
    $desired_ensts{"antisense_".$des_enst} = "antisense_".$output_label;
}
close(DES);
...
my $repeat_working_directory = "/home/elvannostrand/data/clip/CLIPseq_analysis/ALLCLIP_repmapping_20160906/";
#/home/elvannostrand/scratch/ALLCLIP_repmapping_20160906/";
...
for my $k (keys %counts) {
    for my $fi (keys %clip2input) {
        ...
        print "$k\t".$submitted_hash{$fi}{actual}."\t".$submitted_hash{$fi}{cellline};
	
	unless (exists $counts{$k}{$fi}) {
	    $counts{$k}{$fi}{reads} = 0;
	    $counts{$k}{$fi}{rpr} = 0;
	}
        if (exists $counts{$k}{$fi}) {
            print "\t".$counts{$k}{$fi}{reads}."\t".$counts{$k}{$fi}{rpr};

            if (exists $clip2input{$fi}) {
                if (exists $counts{$k}{$clip2input{$fi}}) {
                    my ($clip_n,$clip_rpm) = ($counts{$k}{$fi}{reads},$counts{$k}{$fi}{rpr});
                    my ($input_n,$input_rpm) = ($counts{$k}{$clip2input{$fi}}{reads},$counts{$k}{$clip2input{$fi}}{rpr});
                    unless ($input_rpm > 0) {
                        print STDERR "weird error - fi $fi $clip2input{$fi} $clip_rpm $input_rpm\n";
                    }
                    print "\t".$counts{$k}{$clip2input{$fi}}{reads}."\t".$counts{$k}{$clip2input{$fi}}{rpr}."\t".($clip_rpm / $input_rpm)."\t".&entropy($clip_n,$input_n,$usable_readnum{$fi},$usable_readnum{$clip2input{$fi}});
...
#	    if ($line =~ /Union\t(\d+)$/) {
	    ...
	    my ($global_families,$ensts) = split(/\|\|/,$individ_type);
	    my @ensts_split = split(/\|/,$ensts);

	    my %flag;
	    for my $enst (@ensts_split) {
		if (exists $desired_ensts{$enst}) {
		    $flag{$desired_ensts{$enst}} = 1;
		} else {
		    $flag{"not_found"} = 1;
		}
	    }
	    
	    if (scalar(keys %flag) == 1) {
		if (exists $flag{"not_found"}) {
		    # bad
#		    print STDERR "didn't match correctly $line\n";
		} else {
#		    print STDERR "matched! $line\n";
		    my @keys = keys %flag;
		    my $output_label_new = $keys[0];
		    $counts{$output_label_new}{$label}{reads} += $tmp[2];
		    $counts{$output_label_new}{$label}{rpr} += $tmp[3];
		}
	    }
	}
    }
...
#       for my $rep ("01","02") {                                                                                            
	    ...
	    $submitted_hash{$id}{cellline} = $cellline;
        }
    }
    close(S);
