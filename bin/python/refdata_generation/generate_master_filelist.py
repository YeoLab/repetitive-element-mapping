...
def family_from_gene_name(gene_name):
    """Tier 3 of the Gencode family rule: derive the family from gene_name.

    Returns None when no rule matches, which on hg38 leaves 473 rows for
    --family-override (-475.41). Resolves 831 rows that the rmsk overlap misses.
...
def read_family_overrides(paths):
    """Tier 1 of the Gencode family rule: operator-supplied transcript -> family.

    Highest priority, because it exists to carry knowledge the two derivable
    tiers cannot reach: the 473 hg38 rows whose only Gencode annotation is
    gene_type "snoRNA" under a clone-style gene_name like AC020634.1 (-475.41).
    """
    out = {}
    for path in paths:
        for line in Path(path).read_text().splitlines():
...
def read_family_order(path):
    """The order of the per-family blocks in the Gencode+rRNA region.

    The reference's Gencode region is 32 contiguous runs, one per
    genelists.<FAMILY> source list. Reproducing that order is cosmetic --
    priority_n is compared within a family, so between-family order changes no
    count -- but it keeps generated files diffable against the reference.
    Families absent from the file are appended in first-seen order.
    """
    return [
...
def read_chrom_allowlist(path):
    """Chromosomes a Gencode transcript may live on, pinned per assembly.

    hg38's list was derived from the scaffold set of the reference
    UniqueGenomicElements; the mouse lists are deliberately permissive (every
    sequence in the assembly .fai) pending -475.7.
    """
    return {
        line.strip() for line in Path(path).read_text().splitlines()
        if line.strip() and not line.startswith('#')
...
def read_gtf_lookup(gtf_path):
    """transcript_id -> (gene_id, gene_name, gene_type) for every transcript.

    gene_type is NOT a selection filter -- selecting Gencode rows by transcript
    type is the approach discredited in -475.35. It is read only so that
    build_gencode_rows can tell a genuinely unresolved small RNA from the bulk
    of the GTF, which is meant to be absent.
    """
    out = {}
...
def read_parsed_ucsc_loci(path):
    """transcript_id -> (chrom, txStart, txEnd), first occurrence wins.

    Supplies the coordinates the rmsk small-RNA overlap rule needs, and defines
    which transcripts are candidates at all. File order is preserved so a build
    is deterministic, though the final row order is set by --family-order.
    """
...
def read_curated_class_family(path):
    ...
    RepBase block of the 2020 reference. It exists because 354 of those families
    have no genomic instances in the modern rmsk track (-475.42), so column 4 --
    the label reads are counted under -- would otherwise fall back to the
    family's own name and split e.g. alpha-satellite variants out of centr.

    CIRCULAR: derived from the reference, so exclude it when measuring how well
    an autogenerated hg38 filelist reproduces that reference (-475.22). Same
    derivation style and caveat as refdata/repbase18.05-classes.txt.

    Also the CROSS-SPECIES source: pass the human file to a mouse build and 215
    mouse families inherit by exact family name, which is safe because 167 of
...
def read_rmsk_small_rna_loci(path):
    ...
    A shipped digest of rmsk.txt.gz restricted to repClass in srpRNA, scRNA,
    snRNA, tRNA, rRNA and RNA -- 12,753 rows and 125 KB for hg38, against 155 MB
    for the full table, which is all the overlap rule needs.
    """
...
def overlapping_repname(loci, chrom, start, end, min_frac=0.5):
    """Tier 2 of the Gencode family rule: the rmsk small-RNA feature over a locus.

    Returns the repName of the feature with the largest overlap, but only if it
    covers at least min_frac of the transcript. The 50% floor keeps an
    incidental clip of a neighbouring repeat from claiming a transcript.
...
def read_repeatmasker_names(path):
    ...
    The assembly RepeatMasker GTF supplies the NAME SET and its ORDER only. It
    is stripped to gene_id and carries no repClass/repFamily, which is why
    --rmsk-class-family exists. First-occurrence order is what the block is
    emitted in, so the result is deterministic; do not sort it.
    """
    repeats, simples = [], []
...
def read_mirna_gff3(path):
    ...
    Only primary transcripts: the reference has one pair of rows per miRBase
    hairpin, not per mature arm.
    """
    out, seen = [], set()
    with open(path) as fh:
        for line in fh:
...
def build_gencode_rows(args, log):
    """Block 1: Gencode rows, grouped into per-family blocks.

    Three tiers, tried in order, measured on hg38's 5,261 reference rows:

      1 --family-override                 operator-supplied  (read_family_overrides)
      2 rmsk small-RNA overlap >= 50%     3,932 rows         (overlapping_repname)
    ...
    UNRESOLVED ROWS ARE OMITTED AND COUNTED, NEVER GUESSED. Column 4 is the
    label reads are counted under, so a plausible-but-wrong family is worse than
    an absent row; filling them with the Gencode biotype would reintroduce the
    exact defect T-09 exists to fix. Only transcripts whose gene_type is in
    SMALL_RNA_GENE_TYPES are counted as residue -- the rest of the GTF is meant
    to be absent. See docs/MASTER_FILELIST-decisions.md section 4.
    """
...
def build_rrna_rows(args):
    """Part of block 1: rRNA rows, emitted 18S then 28S then 45S.

    Column 3 is the rDNA COPY identifier (RNA18SN5), read from the GenBank
    /gene="RNA45SN5" qualifier by rrna_copy_suffix -- not a positional index.
    These rows are merged into the Gencode region and placed by --family-order,
    where the reference has them between MTTRNA and RNA5S.
    """
...
def build_repbase_rows(species_fasta, class_family, log, drop_exact_path=None,
    ...
    """Block 2: the RepBase consensus families, NAME | FAM | FAM | FAM | CLASS.

    The family set comes from repbase.select_families(), the same call the index
    makes, so the two artifacts cannot drift apart and the filelist needs no
    index to exist first. Annotation comes from resolve_repeat.

    Unparseable families are FATAL rather than silently dropped -- a missing
    family would silently remove a counting label. --repbase-drop-exact
    overrides the drop list; mouse needs refdata/repbase18.05-drop-exact.mm.txt
    because U7, U8 and U14 each appear twice in the mouse file, once well-formed
    and once as an unparseable bare header (-475.11).
    """
...
def build_simple_repeat_rows():
    """Block 4: the 501 synthesized simple-repeat k-mers.

    Taken from simple_repeat_fasta() so the filelist and the index cannot drift
    apart. Deriving these from the RepeatMasker track instead is the defect that
    gave mm39 17,181 rows against the correct 501.
    """
    return [
...
def build_mirna_rows(gff3_path):
    """Block 5: two rows per miRBase hairpin, the entry and a '-proximal' twin."""
    rows = []
...
def build_rmsk_rows(repeat_names, simple_names, class_family, already,
                    curated_class_family=None):
    """Block 6: everything in the assembly track the RepBase block did not cover.

    Emitted in the track's first-occurrence order. A name already used by block 2
    is skipped HERE ONLY -- rows are never deduplicated across the file as a
    whole, because 61 reference names legitimately appear twice with different
    annotations. (XXX)N simple repeats carry constant Simple_repeat annotation.
    """
    rows = []
