...
#		print STDERR "all $all_reads rep $repfamily_reads genomic $genomic_reads usable $usable_reads\n";
	    } elsif ($tmp[1] eq "UsableReads") {
		$read_sums{usable} += $tmp[2];
	    } elsif ($tmp[1] eq "GenomicReads") {
		$read_sums{genomic} += $tmp[2];
	    } elsif ($tmp[1] eq "RepFamilyReads") {
		$read_sums{repfamily} += $tmp[2];
	    } else {
	    ...
	    my ($ele_delete,$ensg_primary,$readnum,$rpm,$enst_all,$ensg_all) = @tmp;

#	    print "ELEMENT ".join("X x",@tmp)."\n";
