...
my $gencode_gtf_file = $ARGV[3];
my $gencode_tablebrowser_file = $ARGV[4];
my $repmask_bed_fi = $ARGV[5];
my $filelist_file = $ARGV[6];
...
if ($eCLIP_read_type_flag eq "SE") {
    ...
    &read_rep_family_se($repfamily_sam);
    ...
    &read_rep_family_pe($repfamily_sam);
...
close(PREDUP);
close(OUT);
my $count_out = $output_fi.".parsed_v2.20201210.txt";
